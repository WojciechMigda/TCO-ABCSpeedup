...
double GlobalAlignment(const string& s1,
                       const string& s2,
                       double match,
                       double mismatch,
                       double gap,
...
static void NN_chain_core(const t_index N, double * D, vector<Seq> seqs, t_members * const members, cluster_result & Z2) {
    std::size_t ncall = 0;
    static constexpr std::size_t MAGIC = std::numeric_limits<std::size_t>::max();
//    std::cout << "N " << N << std::endl;
    std::vector<std::size_t> access(N * (N - 1), MAGIC);
    const auto __Dref = [&seqs, &N, &D, &ncall, &access](const std::size_t r_, const std::size_t c_) -> t_float &
    {
        const std::size_t index = (static_cast<std::ptrdiff_t>(2*N-3-(r_))*(r_)>>1)+(c_)-1;
        if (access[index] == MAGIC)
        {
            access[index] = ncall;
        }
        else
        {
//            std::cout << /*"W " <<*/ (ncall - access[index]) << std::endl;
            access[index] = ncall;
        }
//        std::cout << index << std::endl;
    ...
    const auto __D = [&seqs, &N, &D, &ncall, &access](const std::size_t r_, const std::size_t c_) -> t_float
    {
        const std::size_t index = (static_cast<std::ptrdiff_t>(2*N-3-(r_))*(r_)>>1)+(c_)-1;
        if (access[index] == MAGIC)
        {
//            access[index] = ncall;
        ...
        {
//            std::cout << index << std::endl; // czy jakis index jest czytany wiecej niz raz po zapisie: TAK
//            std::cout << /*"R " <<*/ (ncall - access[index]) << std::endl;
//            access[index] = ncall;
        }
...
#undef D_
#define D_ref(r_,c_) __Dref(r_, c_)
#define D_(r_,c_) __D(r_, c_)
/*
    N: integer
...
//      std::cerr << "Z2.append " << idx1 << " " << idx2 << " " << min << std::endl;
      ...
      {
          f_average(&D_ref(i, idx2), D_(i, idx1), s, t );
      }
      // Update the distance matrix in the range (idx1, idx2).
      for (; i<idx2; i=active_nodes.succ[i])
      {
          f_average(&D_ref(i, idx2), D_(idx1, i), s, t );
      }
      // Update the distance matrix in the range (idx2, N).
      for (i=active_nodes.succ[idx2]; i<N; i=active_nodes.succ[i])
      {
          f_average(&D_ref(idx2, i), D_(idx1, i), s, t );
      }
...
void linkage(const size_t N, double * matrix, vector<Seq> & seqs, double* Z) {
  //t_index* members = new t_index[N];
  auto_array_ptr<t_index> members;
  members.init(N, 1);
  cluster_result Z2(N - 1);
  NN_chain_core<t_index>(N, matrix, seqs, members, Z2);
...
//
void get_max_dist_for_each_cluster(
    const double *Z, // linkage matrix
    double *max_dists, // output: [n] elementow
    int n
    )
{
    // """
    ...
    //"""
    // zamienia linkage Z w max_dists
...
void form_flat_clusters_from_monotonic_criterion(
    const double *Z, // input, linkage_matrix
    const double mono_crit[], // input, aka. max_dists [n] elementow
    int *T, // output, flat_cluster, wektor przypisan numeru klastra do elementu w seqs, [n] elementow, tyle co w seqs
    double cutoff,
    int n
    )
{
        ...
        if (ndid >= n) {
            if (lid < n) {
                if (ms == -1) {
                    nc++;
                    T[lid] = nc;
//                    	  std::cout << "Cluster # " << nc << ", < " << lid << "," << rid << ">, Dist: " << Zrow[CPY_LIN_DIST] << " Cnt: " << Zrow[CPY_LIN_CNT] << std::endl;
                }
                else {
                    T[lid] = nc;
                }
...
// output(#2): flat_cluster, wektor przypisan numeru klastra do elementu w seqs
//
// input(#1): linkage_matrix
// input(#3): cutoff (jedyne miejsce)

void form_flat_clusters_from_dist(
    const double * Z, // linkage matrix
    int * T, // flat_cluster, wektor przypisan numeru klastra do elementu w seqs, [n] elementow, tyle co w seqs
    double cutoff,
    int n // liczba elementow w seqs (rozmiar T)
    )
{
...
//    cluster_monocrit(Z, max_dists, T, cutoff, n)
    ...
    form_flat_clusters_from_monotonic_criterion(
        Z, // input, linkage_matrix
        max_dists, // input, [n] elementow
        T, // output, [n] elementow
        cutoff, // input
        n // input
        );
...
int analyze(int argc, char **argv) {
  ...
  linkage(seqs.size(), dist_matrix, seqs, linkage_matrix);
  delete []dist_matrix;
